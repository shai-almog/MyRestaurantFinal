...
public class Purchase {
    ...
    public void startOrder() {
        Dialog pleaseWait = new Dialog("Please Wait", new FlowLayout(Component.CENTER));
        pleaseWait.add(new InfiniteProgress());
        pleaseWait.showModeless();
        
        // fetch token from the server
        ConnectionRequest cr = new ConnectionRequest(MyRestaurant.SERVER_URL + "braintree", false);
        cr.addArgument("auth", MyRestaurant.APP_AUTH);
        cr.setFailSilently(true);
        NetworkManager.getInstance().addToQueueAndWait(cr);
        pleaseWait.dispose();
        if(cr.getResponseData() == null || cr.getResponseData().length == 0 || cr.getResponseCode() != 200) {
            ToastBar.showErrorMessage("Failed on authorization server connection, please try again");
            return;
        }
        
        BraintreeNative bn = NativeLookup.create(BraintreeNative.class);
        String token = new String(cr.getResponseData());
        Log.p("Token: " + token);
